...
void Hogweed::collision(Organism* other) {
    world->addLog(this, " has poisoned " + other->getSymbol());
    other->kill();
...
void Hogweed::action() {
   for (size_t i = 0; i < 8; i++){
    auto neighbor = world->getOrganismAt({this->getX() + movement[i][0], this->getY() + movement[i][1]});
    if (neighbor != nullptr) {
        bool isAnimal = dynamic_cast<Animal*>(neighbor);
        if (isAnimal) {
            world->addLog(this, " has poisoned " + neighbor->getSymbol());
            neighbor->kill();
        }
    }
   }
